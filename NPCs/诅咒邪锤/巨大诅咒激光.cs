...
{
	public class 巨大诅咒激光 : ModProjectile
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("巨大诅咒激光");
		}
		...
		public override void AI()
		{
			...
			NPC parent = Main.npc[NPC.FindFirstNPC(ModContent.NPCType<诅咒邪锤>())];
			...
			if (projectile.ai[1] >= 6000f && multiplier == 1f)
			{
				multiplier = -0.5f;
			}
			if (multiplier < 0f && projectile.ai[1] <= 0f || !parent.active)
			{
				projectile.Kill();
			}
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			Texture2D texture = base.mod.GetTexture("NPCs/诅咒邪锤/巨大诅咒激光头");
			Texture2D texture2D = Main.projectileTexture[projectile.type];
			Texture2D texture2 = base.mod.GetTexture("NPCs/诅咒邪锤/巨大诅咒激光尾");
